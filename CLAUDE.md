...
- `auth:username:{sha256}:failedLogins` — attempt budget per username (lowercase hex SHA-256 of the trimmed, upper-cased username, whether or not an account has it), shared by password and MFA-code attempts from every source; `INCR` with the expiry set on the first attempt (`LockoutDurationMinutes`, default 15 min); the failure that reaches `MaxFailedLoginAttempts` holds it with one script (raise to at least the limit, `SET ... PX` the window, recreating an expired key); a correct password that issues an MFA hash gives its own slot back with a floored `DECR`; a completed login `DEL`s it
