...
→ [MFA only] CMFAVerifyPacket → SAuthResultPacket (per-source limit, per-hash attempt limit, TOTP ±1 step, each step once; Status and lock re-checked)
...
**Login throttling** (`CAuthHandler`, `CMFAVerifyHandler`, `SourceBudget`, #471). Every password or MFA-code attempt takes a slot from its source's budget *before* any work, by an atomic increment whose returned value decides: past `MaxFailedLoginsPerSource` inside `FailedLoginSourceWindowMinutes` it is answered `LOCKED` before any account is looked up, so parallel connections cannot all slip past a check made before the first is counted. A correct password or code gives back only its own slot (`DecrementFloorAsync`), never the whole count; an attempt on a locked account keeps its slot. The source is the full IPv4 address, or the /64 prefix for IPv6 (IPv4-mapped addresses count as IPv4); `LastIp`/`LastAttemptIp` store the full address (`RemoteAddress`). An unknown username still costs one BCrypt verify, against `BCryptPasswordVerifier.UnknownAccountHash`, and a wrong password is answered *before* its write, so response time does not reveal which usernames exist. Failed logins are counted in SQL (`IAccountRepository.RecordFailedLoginAsync`, one `UPDATE` whose right-hand sides all read the old row), never by writing back the row that was read, so concurrent failures are not lost; `MaxFailedLoginAttempts` of them lock the account until `LockedUntil` (`LockoutDurationMinutes` later). An expired lock is lifted there too, and by the success write, `TryRecordLoginAsync`, which refuses (and the login is answered `LOCKED`) if the account is locked at write time, so a success can never write away a lock set after it read the row. A lock with `LockedUntil = null` never expires; the migration that added the column gave every earlier lock a 15-minute end. The lock check stays before the password check on purpose: after it, a locked account would still tell right from wrong passwords. Each MFA hash allows `MaxFailedMfaAttempts` codes, counted on the `auth:account:{accountId}:mfa` hash before the code is checked; the last failure deletes the hash, so the client must log in again. Because each password login makes a fresh hash, every wrong code also counts towards the account lock through `RecordFailedLoginAsync` (answered first, written after, like a wrong password), and a code whose hash is gone (no reverse key, or `attempts` of -1) is refused without being checked. All these settings are under `Application` in the Auth server's config.
