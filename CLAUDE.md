...
- `auth:account:{accountId}:mfa` — Redis Hash with MFA state (2 min TTL); its `attempts` field counts MFA codes tried against it (`HINCRBY`, only while the hash exists)
- `auth:source:{source}:failedLogins` — attempt budget per source (IPv4 address, or IPv6 /64 as `prefix::/64`), `INCR` with the expiry set on the first attempt (fixed window, default 15 min); a correct password or MFA code gives its own slot back with a floored `DECR`
- `auth:username:{sha256}:failedLogins` — attempt budget per username (lowercase hex SHA-256 of the trimmed, upper-cased username, whether or not an account has it), shared by password and MFA-code attempts from every source; `INCR` with the expiry set on the first attempt (`LockoutDurationMinutes`, default 15 min); the failure that reaches `MaxFailedLoginAttempts` holds it with one script (raise to at least the limit + 1, `SET ... PX` the window, recreating an expired key); a correct password that issues an MFA hash gives its own slot back with one script that decrements only while `0 < v <= limit` (never lowering a held value); a completed login deletes it with one script, only while its value is below the held value, limit + 1 (so a reset never deletes a hold)
