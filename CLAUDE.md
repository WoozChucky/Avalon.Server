...
**The account lock is a per-username budget** (`UsernameBudget`, #484), built on the same `AttemptBudget` mechanics as the source budget. Every password attempt takes a slot keyed by the username as it is looked up (trimmed, upper-cased, then SHA-256 in the key) *before* `FindByUserNameAsync`, from every source alike and whether or not an account has that username, and every MFA code takes one from its account's username before the code is checked. The count the increment returns is the attempt's place in the window (`LockoutDurationMinutes`, fixed from the first attempt), and it alone decides, never the row as read: past `MaxFailedLoginAttempts` the attempt is answered `LOCKED` before the lookup; a wrong password (or wrong code) in the last slot is answered `LOCKED`, restarts the key's expiry (so the refusal ends with the row's lock) and locks the row; an earlier one is answered `INVALID_CREDENTIALS` (`MFA_FAILED`). An unknown username goes through exactly the same steps, answers and expiry, minus the database write. Before it, the lock was decided from the row each request read before its BCrypt verify, so parallel guesses spread over sources got about ten verifies per lockout, and the right password, reaching its success write after the batch's failures had locked the row, was the one answered `LOCKED`. A correct password or code gives back only its own slot, so a success does not reset the count others made. Failed logins are still counted in SQL (`IAccountRepository.RecordFailedLoginAsync`, one `UPDATE` whose right-hand sides all read the old row), never by writing back the row that was read, so concurrent failures are not lost; the caller passes `lockUntil` when the budget says the failure locks, and the row stays locked until `LockedUntil`. An expired lock is lifted there too, and by the success write, `TryRecordLoginAsync`, which refuses if the account is locked at write time, so a success can never write away a lock set after it read the row; that refusal is answered as a wrong password in the same slot would be (`INVALID_CREDENTIALS`/`MFA_FAILED`, or `LOCKED` in the last slot), so it never singles out the right password in a batch. A lock with `LockedUntil = null` never expires; the migration that added the column gave every earlier lock a 15-minute end. The row's lock check stays before the password (and code) check on purpose: after it, a locked account would still tell right from wrong passwords. Each MFA hash allows `MaxFailedMfaAttempts` codes, counted on the `auth:account:{accountId}:mfa` hash before the code is checked; the last failure deletes the hash, so the client must log in again, and a code whose hash is gone (no reverse key, or `attempts` of -1) is refused without being checked. All these settings are under `Application` in the Auth server's config.
...
- `auth:username:{sha256}:failedLogins` — attempt budget per username (lowercase hex SHA-256 of the trimmed, upper-cased username, whether or not an account has it), shared by password and MFA-code attempts from every source; `INCR` with the expiry set on the first attempt (`LockoutDurationMinutes`, default 15 min) and restarted by the failure that reaches `MaxFailedLoginAttempts`; a correct password or MFA code gives its own slot back with a floored `DECR`
