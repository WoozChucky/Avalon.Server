...
- **The seeded loot** (migration `SeedForestLoot`): hostile templates 4-10 each name their own table (2-8) and a copper range; the town NPCs (1-3) name none. Each creature table rolls a Health Potion and a Mana Potion on their own, the shared table 1 "Forest common", and table 9 "Forest weapons" at 10 %, which is one group (Rusted Sword 34, Splintered Staff 33, Warped Shortbow 33), so a kill drops at most one weapon. Chances rise with rarity. `SeedIntegrityShould` pins all of it.
