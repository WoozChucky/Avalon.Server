...
- **CreatureStatDeriver** (`Avalon.World/Creatures/`) — turns a creature template plus a rolled level into the stats it spawns with: `CreatureBaseStats` by level, scaled by the template's modifiers and its `CreatureRarity` via `CreatureRarityModifiers`. `CreatureSpawner` calls it; before it existed every creature spawned as level 1 with 100 health whatever its template said. Injected as a `Lazy` because the spawner loads before `StaticData` does. `CreatureTemplate.Exp` overrides the derived experience when set — `null` means derive.
