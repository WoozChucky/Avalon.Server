...
public static class rlImGui
{
    ...
    internal static bool rlImGuiIsControlDown() =>
        IsKeyDown(KeyboardKey.RightControl) || IsKeyDown(KeyboardKey.LeftControl);

    internal static bool rlImGuiIsShiftDown() => IsKeyDown(KeyboardKey.RightShift) || IsKeyDown(KeyboardKey.LeftShift);
    internal static bool rlImGuiIsAltDown() => IsKeyDown(KeyboardKey.RightAlt) || IsKeyDown(KeyboardKey.LeftAlt);
    internal static bool rlImGuiIsSuperDown() => IsKeyDown(KeyboardKey.RightSuper) || IsKeyDown(KeyboardKey.LeftSuper);
    ...
    public static unsafe void ReloadFonts()
    {
        ...
        //if (Raylib_cs.Raylib.IsTextureReady(FontTexture))
        //{
    ...
    internal static unsafe sbyte* rImGuiGetClipText(IntPtr userData) => GetClipboardText();

    internal static unsafe void rlImGuiSetClipText(IntPtr userData, sbyte* text) => SetClipboardText(text);
    ...
    public static void EndInitImGui()
    {
        ...
        unsafe
        {
            ...
            fixed (ushort* range = &IconRanges[0])
            {
                ...
                FontAwesome6.IconFontRanges = Marshal.AllocHGlobal(6);
                ...
                {
                    ImFontPtr fontPtr = ImGui.GetIO().Fonts.AddFontFromMemoryTTF(new IntPtr(buffer),
            ...
            if (imgui_cursor != CurrentMouseCursor || io.MouseDrawCursor)
            {
                ...
                else
                {
                    ...
                    if ((io.ConfigFlags & ImGuiConfigFlags.NoMouseCursorChange) == 0)
                    {
                        ...
                        {
                            SetMouseCursor(MouseCursorMap[imgui_cursor]);
                        }
                    }
                }
            }
    ...
    private static void FrameEvents()
    {
        ...
        foreach (KeyValuePair<KeyboardKey, ImGuiKey> keyItr in RaylibKeyMap)
        {
    ...
    public static void Shutdown()
    {
        ...
        {
            if (FontAwesome6.IconFontRanges != IntPtr.Zero)
            {
                Marshal.FreeHGlobal(FontAwesome6.IconFontRanges);
            }
    ...
    ///     The portion of the texture to draw as an image. Negative values for the width and height will
    ///     flip the image
    /// </param>
    ...
    public static void ImageRenderTexture(RenderTexture2D image) => ImageRect(image.Texture, image.Texture.Width,
        image.Texture.Height, new Rectangle(0, 0, image.Texture.Width, -image.Texture.Height));

    /// <summary>
    ///     Draws a render texture as an image to the current ImGui Context, flipping the Y axis so it will show correctly on
    ///     the screen
    ///     The texture will be scaled to fit the content are available, centered if desired
    /// </summary>
    /// <param name="image">The render texture to draw</param>
    /// <param name="center">
    ///     When true the texture will be centered in the content area. When false the image will be left and
    ///     top justified
    /// </param>
