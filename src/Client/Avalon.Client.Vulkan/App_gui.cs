// Licensed to the Avalon MMORPG Game under one or more agreements.
// Avalon MMORPG Game licenses this file to you under the MIT license.

using Avalon.Client.Vulkan.Engine;
using Avalon.Client.Vulkan.Systems.PointLight;

namespace Avalon.Client.Vulkan;

public partial class App
{
    private readonly Vector2 buttonSize = new(80, 24);
    private readonly long interval = 10_000 * 1000; // 250 ms

    // footer props
    private readonly string status = "";
    private Vector2 bl;
    private Vector2 br;
    private Vector2 cPos;
    private Vector2 footerPos;
    private Vector2 footerSize;
    private long lastSample;
    private float lightIntensity;
    private float lightRadius;
    private float memory;

    // pointLight props
    private float rotateSpeed = 1f;
    private Vector2 tl;
    private Vector2 toolbarPos;
    private Vector2 toolbarSize;
    private Vector2 tr;

    // mesh shader props
    private int triangleCount = 3;
    private float triangleHeight = 1;
    private float triangleSpacing = 1;
    private float triangleWidth = 1;
    private Vector2 wPos;
    private Vector2 wSize;
    private float xc;
    private float yc;
    private float yPosition;

    private void FirstAppGuiInit()
    {
        rotateSpeed = pointLightRenderSystem.RotateSpeed;
        LveGameObject? yCheck = gameObjects.Values.FirstOrDefault(s => s.PointLight.HasValue);
        if (yCheck is not null)
        {
            yPosition = yCheck.Transform.Translation.Y;
            lightIntensity = yCheck.PointLight?.LightIntensity ?? 0f;
            lightRadius = yCheck.Transform.Scale.X;
        }

        triangleCount = (int)sboMeshTest.TriangleCount;
        triangleSpacing = sboMeshTest.TriangleSpacing;
        triangleHeight = sboMeshTest.TriangleHeight;
        triangleWidth = sboMeshTest.TriangleWidth;
    }

    private void FirstAppGuiUpdate()
    {
        ImGui.SetNextWindowPos(toolbarPos);
        ImGui.SetNextWindowSize(toolbarSize);
        ImGui.Begin("MainToolbar",
            ImGuiWindowFlags.NoBackground
            | ImGuiWindowFlags.NoTitleBar
            | ImGuiWindowFlags.NoResize
            | ImGuiWindowFlags.NoMove
            | ImGuiWindowFlags.NoSavedSettings
        );
        {
            ImGui.Text("Hello");
            ImGui.SameLine();
            if (ImGui.Button("GC", buttonSize))
            {
                GC.Collect();
            }
        }
        ImGui.End();


        ImGui.Begin("Settings");
        {
            // Rotate speed
            ImGui.Text("Rotate Speed Factor");
            if (ImGui.SliderFloat("##rotateSpeed", ref rotateSpeed, -5f, 5f))
            {
                pointLightRenderSystem.RotateSpeed = rotateSpeed;
            }

            ImGui.SameLine();
            if (ImGui.Button("Stop##rotateSpeedZero"))
            {
                rotateSpeed = 0f;
                pointLightRenderSystem.RotateSpeed = rotateSpeed;
            }

            // Y Position
            ImGui.Text("Y Position");
            if (ImGui.SliderFloat("##yPosition", ref yPosition, -.5f, 2.5f))
            {
                foreach (LveGameObject mod in gameObjects.Values.Where(s => s.PointLight.HasValue))
                {
                    mod.Transform.Translation.Y = yPosition;
                }
            }

            ImGui.SameLine();
            if (ImGui.Button("Reset##yPositionZero"))
            {
                yPosition = 0f;
                foreach (LveGameObject mod in gameObjects.Values.Where(s => s.PointLight.HasValue))
                {
                    mod.Transform.Translation.Y = yPosition;
                }
            }

            // Light Intensity
            ImGui.Text("Light Intensity");
            if (ImGui.SliderFloat("##lightIntensity", ref lightIntensity, 0.005f, 1.000f))
            {
                foreach (LveGameObject mod in gameObjects.Values.Where(s => s.PointLight.HasValue))
                {
                    mod.PointLight = new PointLightComponent(lightIntensity);
                }
            }

            ImGui.SameLine();
            if (ImGui.Button("Reset##lightIntensityZero"))
            {
                lightIntensity = 0f;
                foreach (LveGameObject mod in gameObjects.Values.Where(s => s.PointLight.HasValue))
                {
                    mod.PointLight = new PointLightComponent(lightIntensity);
                }
            }

            // Light Radius
            ImGui.Text("Light Radius");
            if (ImGui.SliderFloat("##lightRadius", ref lightRadius, 0.01f, 0.5f))
            {
                foreach (LveGameObject mod in gameObjects.Values.Where(s => s.PointLight.HasValue))
                {
                    mod.Transform.Scale.X = lightRadius;
                }
            }

            ImGui.SameLine();
            if (ImGui.Button("Reset##lightRadiusZero"))
            {
                lightRadius = 0f;
                foreach (LveGameObject mod in gameObjects.Values.Where(s => s.PointLight.HasValue))
                {
                    mod.Transform.Scale.X = lightRadius;
                }
            }


            // triangle mesh shader stuff
            ImGui.NewLine();
            ImGui.Separator();
            ImGui.Text("Triangle Count");
            if (ImGui.SliderInt("##triangleCount", ref triangleCount, 1, 100))
            {
                sboMeshTest = sboMeshTest with {TriangleCount = (uint)triangleCount};
                sboMeshTestBuffer.WriteToBuffer(sboMeshTest);
            }

            ImGui.Text("Triangle Spacing");
            if (ImGui.SliderFloat("##triangleSpacing", ref triangleSpacing, 0.1f, 5f))
            {
                sboMeshTest = sboMeshTest with {TriangleSpacing = triangleSpacing};
                sboMeshTestBuffer.WriteToBuffer(sboMeshTest);
            }

            // haven't implemented height/width yet
            //ImGui.Text("Triangle Height");
            //if (ImGui.SliderFloat("##triangleHeight", ref triangleHeight, 0.1f, 5f))
            //{
            //    sboMeshTest = sboMeshTest with { TriangleHeight = triangleHeight };
            //    sboMeshTestBuffer.WriteToBuffer(sboMeshTest);
            //}
            //ImGui.Text("Triangle Width");
            //if (ImGui.SliderFloat("##triangleWidth", ref triangleWidth, 0.1f, 5f))
            //{
            //    sboMeshTest = sboMeshTest with { TriangleWidth = triangleWidth };
            //    sboMeshTestBuffer.WriteToBuffer(sboMeshTest);
            //}
        }
        ImGui.End();


        // footer
        long tick = DateTime.Now.Ticks;

        if (tick - lastSample > interval)
        {
            lastSample = tick;
            memory = Process.GetCurrentProcess().WorkingSet64 / 1_000_000;
        }

        ImGui.SetNextWindowPos(footerPos);
        ImGui.SetNextWindowSize(footerSize);
        ImGui.Begin("Status", ImGuiWindowFlags.NoBackground
                              | ImGuiWindowFlags.NoTitleBar
                              | ImGuiWindowFlags.NoResize
                              | ImGuiWindowFlags.NoMove
                              | ImGuiWindowFlags.NoSavedSettings
                              | ImGuiWindowFlags.NoInputs);
        {
            string fps = $"FPS {ImGui.GetIO().Framerate,7:0.00}";
            string mems = $"MEM {memory,3:#,#}MB";
            string m2 =
                $"M2d {mouseLast.Pos2d.X,6:+0.0000;-0.0000;0.0000},{mouseLast.Pos2d.Y,6:+0.0000;-0.0000;0.0000}";
            //var mhud = $"Mhud {mouseHud.X,7:+0.000;-0.000},{mouseHud.Y,7:+0.000;-0.000}";
            string m3 =
                $"M3d {mouseLast.Pos3d.X,7:+0.000;-0.000;0.0000},{mouseLast.Pos3d.Y,7:+0.000;-0.000;0.0000},{mouseLast.Pos3d.Z,7:+0.000;-0.000;0.0000}";
            string v = $"V {window.FramebufferSize.X,4:0}x{window.FramebufferSize.Y,-4:0}";
            //var f = $"F {camera.Frustum}";
            //var p = $"P {camera.Position.X:0.000},{camera.Position.Y:0.000},{camera.Position.Z:0.000}";
            //var yaw = $"Yaw {camera.Yaw:0.000}";
            //var pitch = $"Pitch {camera.Pitch:0.000}";
            //ImGui.Text($"{status,-50} | {fps,-8} | {mems,-12} | {raw,-15} | {m2,-12} | {mhud,-12} | {m3,-22} | {v,-12} | {f,-6} | {p,-16} | {yaw,-6} | {pitch,-6}");
            ImGui.Text($"{status,-20} | {fps} | {mems} | {m2} | {m3} | {v}");
        }

        ImGui.End();
    }


    public void FirstAppGuiResize(int xp, int yp, uint wp, uint hp, Vector2D<int> size)
    {
        wPos = new Vector2(xp - 10, yp + 10);
        wSize = new Vector2(wp + 20, hp + 20);
        xc = xp + wp / 2f;
        yc = yp + hp / 2f;
        tl = new Vector2(xp, yp);
        bl = new Vector2(xp, yp + hp);
        tr = new Vector2(xp + wp, yp);
        br = new Vector2(xp + wp, yp + hp);

        cPos = new Vector2(xp + wp - 200, yp + hp - 20);

        toolbarPos = new Vector2(xp + 5, yp + 5);
        toolbarSize = new Vector2(wp - 10, 40);

        footerPos = new Vector2(10, size.Y - 25);
        footerSize = new Vector2(size.X - 20, 25);
    }
}
