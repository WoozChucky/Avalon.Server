...
public class LveStdPipeline : IDisposable
{
    ...
    public LveStdPipeline(Vk vk, LveDevice device, string vertPath, string fragPath, PipelineConfigInfo configInfo,
    ...
    public GraphicsPipelineTypes PipelineType { get; } = GraphicsPipelineTypes.Std;
    ...
    private unsafe void createGraphicsPipelineStd(string vertPath, string fragPath, PipelineConfigInfo configInfo,
        string renderSystemName)
    {
        byte[] vertBytes = FileUtil.GetShaderBytes(vertPath, renderSystemName);
        byte[] fragBytes = FileUtil.GetShaderBytes(fragPath, renderSystemName);
        ...
        PipelineShaderStageCreateInfo* shaderStages = stackalloc[] {vertShaderStageInfo, fragShaderStageInfo};
        ...
        {
            PipelineVertexInputStateCreateInfo vertextInputInfo = new()
            {
            ...
            if (vk.CreateGraphicsPipelines(device.VkDevice, default, 1, pipelineInfo, default, out graphicsPipeline) !=
                Result.Success)
            {
    ...
    private unsafe void createGraphicsPipelineMesh(string taskPath, string meshPath, string fragPath,
        PipelineConfigInfo configInfo, string renderSystemName)
    {
        byte[] taskBytes = FileUtil.GetShaderBytes(taskPath, renderSystemName);
        byte[] meshBytes = FileUtil.GetShaderBytes(meshPath, renderSystemName);
        byte[] fragBytes = FileUtil.GetShaderBytes(fragPath, renderSystemName);
        ...
        {
            meshShaderStageInfo, taskShaderStageInfo, fragShaderStageInfo
        };
    ...
    public static unsafe void DefaultPipelineConfigInfo(ref PipelineConfigInfo configInfo)
    {
        ...
        configInfo.ColorBlendInfo.PAttachments =
            (PipelineColorBlendAttachmentState*)Unsafe.AsPointer(ref configInfo.ColorBlendAttachment);
