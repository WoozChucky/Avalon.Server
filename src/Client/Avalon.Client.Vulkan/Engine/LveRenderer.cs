...
public class LveRenderer : IDisposable
{
    ...
    private unsafe void freeCommandBuffers()
    {
        fixed (CommandBuffer* commandBuffersPtr = commandBuffers)
        {
            vk.FreeCommandBuffers(device.VkDevice, device.GetCommandPool(), (uint)commandBuffers.Length,
                commandBuffersPtr);
        }
    ...
    public unsafe void BeginSwapChainRenderPass(CommandBuffer commandBuffer)
    {
        ...
        Debug.Assert(commandBuffer.Handle == GetCurrentCommandBuffer().Handle,
        ...
        RenderPassBeginInfo renderPassInfo = new()
        {
            ...
            RenderArea = {Offset = {X = 0, Y = 0}, Extent = swapChain.GetSwapChainExtent()}
        };
        ...
        ClearValue[] clearValues = new ClearValue[]
        {
            new()
            {
                ...
                {
                    Float32_0 = 0.01f, Float32_1 = 0.01f, Float32_2 = 0.01f, Float32_3 = 1
                }
            },
    ...
    private Extent2D GetWindowExtents() => new((uint)window.FramebufferSize.X, (uint)window.FramebufferSize.Y);
}
