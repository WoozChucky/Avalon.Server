...
public class PointLightRenderSystem : IDisposable
{
    ...
    private readonly Dictionary<uint, float> sortedZ = new();
    ...
    {
        int lightIndex = 0;
        foreach ((uint idx, LveGameObject? g) in frameInfo.GameObjects)
        {
            if (g.PointLight is null)
            {
                continue;
            }
            ...
            {
                Matrix4x4 rotateLight = Matrix4x4.CreateRotationY(frameInfo.FrameTime * RotateSpeed);
    ...
    public unsafe void Render(FrameInfo frameInfo)
    {
        ...
        foreach ((uint idx, LveGameObject? g) in frameInfo.GameObjects)
        {
            ...
            Vector3 offset = frameInfo.Camera.FrontVec - g.Transform.Translation;
        ...
        foreach ((uint gid, float dist) in sortedZ.OrderBy(s => s.Value))
        {
            LveGameObject? g = frameInfo.GameObjects[gid];
            if (g.PointLight is null)
            {
                continue;
            }
            ...
            {
                Position = new Vector4(g.Transform.Translation, 1f),
                Color = new Vector4(g.Color.X, g.Color.Y, g.Color.Z, g.PointLight.Value.LightIntensity),
