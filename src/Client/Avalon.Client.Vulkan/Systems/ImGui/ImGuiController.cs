...
public class ImGuiController : IDisposable
{
    ...
    public unsafe ImGuiController(Vk vk, IView view, IInputContext input, ImGuiFontConfig imGuiFontConfig,
        ...
        Format? depthBufferFormat, SampleCountFlags msaaSamples)
    {
        IntPtr context = ImGuiNET.ImGui.CreateContext();
        ImGuiNET.ImGui.SetCurrentContext(context);
        ...
        Format swapChainFormat, Format? depthBufferFormat, SampleCountFlags msaaSamples)
    {
        ...
        Span<DescriptorPoolSize> poolSizes =
            stackalloc DescriptorPoolSize[] {new DescriptorPoolSize(DescriptorType.CombinedImageSampler, 1)};
        DescriptorPoolCreateInfo descriptorPool = new();
        ...
        {
            throw new ApplicationException("ImGui Depth Format must have a value!");
        }
        ...
        {
            DescriptorSetAllocateInfo alloc_info = new();
        ...
        PushConstantRange vertPushConst = new();
        vertPushConst.StageFlags = ShaderStageFlags.VertexBit;
        vertPushConst.Offset = sizeof(float) * 0;
        vertPushConst.Size = sizeof(float) * 4;

        DescriptorSetLayout set_layout = _descriptorSetLayout;
        PipelineLayoutCreateInfo layout_info = new();
        ...
        if (_shaderModuleVert.Handle == default)
        {
            ...
            {
                ShaderModuleCreateInfo vert_info = new();
        ...
        if (_shaderModuleFrag.Handle == default)
        {
            ...
            {
                ShaderModuleCreateInfo frag_info = new();
        ...
        VertexInputBindingDescription binding_desc = new();
        ...
        PipelineVertexInputStateCreateInfo vertex_info = new();
        ...
        vertex_info.PVertexAttributeDescriptions =
            (VertexInputAttributeDescription*)Unsafe.AsPointer(ref attribute_desc[0]);

        PipelineInputAssemblyStateCreateInfo ia_info = new();
        ia_info.SType = StructureType.PipelineInputAssemblyStateCreateInfo;
        ia_info.Topology = PrimitiveTopology.TriangleList;

        PipelineViewportStateCreateInfo viewport_info = new();
        viewport_info.SType = StructureType.PipelineViewportStateCreateInfo;
        viewport_info.ViewportCount = 1;
        viewport_info.ScissorCount = 1;

        PipelineRasterizationStateCreateInfo raster_info = new();
        ...
        PipelineMultisampleStateCreateInfo ms_info = new();
        ms_info.SType = StructureType.PipelineMultisampleStateCreateInfo;
        ms_info.RasterizationSamples = msaaSamples;

        PipelineColorBlendAttachmentState color_attachment = new();
        color_attachment.BlendEnable = new Bool32(true);
        ...
        PipelineDepthStencilStateCreateInfo depth_info = new();
        depth_info.SType = StructureType.PipelineDepthStencilStateCreateInfo;

        PipelineColorBlendStateCreateInfo blend_info = new();
        ...
        PipelineDynamicStateCreateInfo dynamic_state = new();
        ...
        if (_vk.CreateCommandPool(_device, poolInfo, null, out CommandPool commandPool) != Result.Success)
        {
        ...
        _vk.GetImageMemoryRequirements(_device, _fontImage, out MemoryRequirements fontReq);
        MemoryAllocateInfo fontAllocInfo = new();
        fontAllocInfo.SType = StructureType.MemoryAllocateInfo;
        fontAllocInfo.AllocationSize = fontReq.Size;
        fontAllocInfo.MemoryTypeIndex =
            GetMemoryTypeIndex(vk, MemoryPropertyFlags.DeviceLocalBit, fontReq.MemoryTypeBits);
        ...
        DescriptorImageInfo descImageInfo = new();
        ...
        WriteDescriptorSet writeDescriptors = new();
        ...
        if (_vk.CreateBuffer(_device, bufferInfo, default, out Buffer uploadBuffer) != Result.Success)
        {
            throw new Exception("Failed to create a device buffer");
        }

        _vk.GetBufferMemoryRequirements(_device, uploadBuffer, out MemoryRequirements uploadReq);
        _bufferMemoryAlignment =
            _bufferMemoryAlignment > uploadReq.Alignment ? _bufferMemoryAlignment : uploadReq.Alignment;

        MemoryAllocateInfo uploadAllocInfo = new();
        uploadAllocInfo.SType = StructureType.MemoryAllocateInfo;
        uploadAllocInfo.AllocationSize = uploadReq.Size;
        uploadAllocInfo.MemoryTypeIndex =
            GetMemoryTypeIndex(vk, MemoryPropertyFlags.HostVisibleBit, uploadReq.MemoryTypeBits);
        if (_vk.AllocateMemory(_device, uploadAllocInfo, default, out DeviceMemory uploadBufferMemory) !=
            Result.Success)
        {
            throw new Exception("Failed to allocate device memory");
        }
        ...
        ImageMemoryBarrier copyBarrier = new();
        ...
        ImageMemoryBarrier use_barrier = new();
        ...
        _vk.GetDeviceQueue(_device, graphicsFamilyIndex, 0, out Queue graphicsQueue);
    ...
    {
        vk.GetPhysicalDeviceMemoryProperties(_physicalDevice, out PhysicalDeviceMemoryProperties prop);
    ...
    private static void SetKeyMappings()
    {
        ...
        //io.KeysData[(int)ImGuiKey.Tab] = new ImGuiKeyData {Down = 1};
    }
        ...
        in Framebuffer framebuffer, in Extent2D swapChainExtent)
    {
        ...
        ImDrawData drawData = *drawDataPtr.NativePtr;
        ...
        ref FrameRenderBuffer frameRenderBuffer =
            ref _mainWindowRenderBuffers.FrameRenderBuffers[_mainWindowRenderBuffers.Index];
