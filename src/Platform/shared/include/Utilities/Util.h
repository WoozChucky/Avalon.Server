...
Optional<S32> MoneyStringToMoney(std::string_view moneyString);

std::string secsToTimeString(U64 timeInSecs, bool shortText = false);
U32 TimeStringToSecs(const std::string& timestring);
...
inline bool IsEvenNumber(S32 n)
{
    return n % 2 == 0;
}
...
inline wchar_t wcharToUpper(wchar_t wchar)
{
    if (wchar >= L'a' && wchar <= L'z')                      // LATIN SMALL LETTER A - LATIN SMALL LETTER Z
    {
        return wchar_t(U16(wchar) - 0x0020);
    }
    if (wchar == 0x00DF)                                     // LATIN SMALL LETTER SHARP S
    {
    ...
    if (wchar >= 0x0101 && wchar <= 0x012F)                  // LATIN SMALL LETTER A WITH MACRON - LATIN SMALL LETTER I WITH OGONEK (only %2=1)
    {
        if (wchar % 2 == 1)
        {
            return wchar_t(U16(wchar) - 0x0001);
        }
    }
...
std::wstring GetMainPartOfName(std::wstring const& wname, U32 declension);
...
U32 CreatePIDFile(const std::string& filename);
U32 GetPID();

namespace Avalon::Impl
{
    std::string ByteArrayToHexStr(U8 const* bytes, size_t length, bool reverse = false);
    void HexStrToByteArray(std::string_view str, U8* out, size_t outlen, bool reverse = false);
}

template<typename Container>
std::string ByteArrayToHexStr(Container const& c, bool reverse = false)
{
    return Avalon::Impl::ByteArrayToHexStr(std::data(c), std::size(c), reverse);
}

template<size_t Size>
void HexStrToByteArray(std::string_view str, std::array<U8, Size>& buf, bool reverse = false)
{
    Avalon::Impl::HexStrToByteArray(str, buf.data(), Size, reverse);
}

template<size_t Size>
std::array<U8, Size> HexStrToByteArray(std::string_view str, bool reverse = false)
{
...
public:
    flag96(U32 p1 = 0, U32 p2 = 0, U32 p3 = 0)
    {
        part[0] = p1;
        part[1] = p2;
        part[2] = p3;
    ...
    [[nodiscard]] inline bool IsEqual(U32 p1 = 0, U32 p2 = 0, U32 p3 = 0) const
    {
        return (part[0] == p1 && part[1] == p2 && part[2] == p3);
    }

    [[nodiscard]] inline bool HasFlag(U32 p1 = 0, U32 p2 = 0, U32 p3 = 0) const
    {
        return (part[0] & p1 || part[1] & p2 || part[2] & p3);
    }

    inline void Set(U32 p1 = 0, U32 p2 = 0, U32 p3 = 0)
    {
        part[0] = p1;
        part[1] = p2;
        part[2] = p3;
    ...
    inline U32& operator[](U8 el)
    {
        return part[el];
    }

    inline U32 const& operator [](U8 el) const
    {
        return part[el];
    }
};
