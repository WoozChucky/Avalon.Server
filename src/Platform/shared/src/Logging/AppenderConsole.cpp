...
AppenderConsole::AppenderConsole(U8 id, std::string const& name, LogLevel level, AppenderFlags flags, std::vector<std::string_view> const& args)
    : Appender(id, name, level, flags), _colored(false)
{
    for (U8 i = 0; i < NUM_ENABLED_LOG_LEVELS; ++i)
    {
...
void AppenderConsole::InitColors(std::string const& name, std::string_view str)
{
    ...
    std::vector<std::string_view> colorStrs = Avalon::Tokenize(str, ' ', false);
    if (colorStrs.size() != NUM_ENABLED_LOG_LEVELS)
    {
        throw InvalidAppenderArgsException(Avalon::StringFormatFmt("Log::CreateAppenderFromConfig: Invalid color data '{}' for console appender {} (expected {} entries, got {})",
            str, name, NUM_ENABLED_LOG_LEVELS, colorStrs.size()));
    }

    for (U8 i = 0; i < NUM_ENABLED_LOG_LEVELS; ++i)
    {
        if (Optional<U8> color = Avalon::StringTo<U8>(colorStrs[i]); color && EnumUtils::IsValid<ColorTypes>(*color))
        {
            _colors[i] = static_cast<ColorTypes>(*color);
        }
        else
        {
            throw InvalidAppenderArgsException(Avalon::StringFormatFmt("Log::CreateAppenderFromConfig: Invalid color '{}' for log level {} on console appender {}",
