...
Appender::Appender(U8 _id, std::string const& _name, LogLevel _level /* = LOG_LEVEL_DISABLED */, AppenderFlags _flags /* = APPENDER_FLAGS_NONE */):
    id(_id), name(_name), level(_level), flags(_flags) { }

Appender::~Appender() { }

U8 Appender::getId() const
{
    return id;
}
...
void Appender::write(LogMessage* message)
{
    ...
    {
        ss << Avalon::StringFormat("%-5s ", Appender::getLogLevelString(message->level));
    }
