...
{
    message().assign(Avalon::StringFormat("Invalid %s value (%s) found in ByteBuffer", type, value));
}
...
U32 ByteBuffer::ReadPackedTime()
{
    U32 packedDate = read<U32>();
    tm lt = tm();
    ...
    return U32(mktime(&lt));
}

void ByteBuffer::append(U8 const* src, size_t cnt)
{
...
{
    tm lt = Avalon::Time::TimeBreakdown(time);
    append<U32>((lt.tm_year - 100) << 24 | lt.tm_mon << 20 | (lt.tm_mday - 1) << 14 | lt.tm_wday << 11 | lt.tm_hour << 6 | lt.tm_min);
}

void ByteBuffer::put(size_t pos, U8 const* src, size_t cnt)
{
