...
void IpLocationStore::Load()
{
    ...
    while (databaseFile.good())
    {
        ...
        auto IpFrom = Avalon::StringTo<U32>(ipFrom);
        auto IpTo = Avalon::StringTo<U32>(ipTo);
    ...
    LOG_INFO("server.loading", ">> Loaded {} ip location entries.", static_cast<U32>(_ipLocationStore.size()));
    LOG_INFO("server.loading", " ");
}

IpLocationRecord const* IpLocationStore::GetLocationRecord(std::string const& ipAddress) const
{
    U32 ip = Avalon::Net::address_to_uint(Avalon::Net::make_address_v4(ipAddress));
    auto itr = std::upper_bound(_ipLocationStore.begin(), _ipLocationStore.end(), ip, [](U32 ip, IpLocationRecord const& loc) { return ip < loc.IpTo; });
