...
#include <Common/Utilities/Containers.h>
...
std::string secsToTimeString(U64 timeInSecs, bool shortText)
{
    U64 secs    = timeInSecs % MINUTE;
    U64 minutes = timeInSecs % HOUR / MINUTE;
    U64 hours   = timeInSecs % DAY  / HOUR;
    U64 days    = timeInSecs / DAY;
...
Optional<S32> MoneyStringToMoney(std::string_view moneyString)
{
    ...
    for (std::string_view token : Avalon::Tokenize(moneyString, ' ', false))
    {
        U32 unit;
        ...
        Optional<U32> amount = Avalon::StringTo<U32>(token.substr(0, token.length() - 1));
        if (amount)
        {
            money += (unit * *amount);
        }
...
bool IsIPAddress(char const* ipaddress)
{
    ...
    Avalon::Net::make_address(ipaddress, error);
    return !error;
}

/// create PID file
U32 CreatePIDFile(std::string const& filename)
{
...
bool Utf8toWStr(char const* utf8str, size_t csize, wchar_t* wstr, size_t& wsize)
{
    try
    {
        Avalon::CheckedBufferOutputIterator<wchar_t> out(wstr, wsize);
...
void vutf8printf(FILE* out, const char* str, va_list* ap)
{
    ...
    CharToOemBuffW(&wtemp_buf[0], &temp_buf[0], U32(wtemp_len + 1));
...
std::string Avalon::Impl::ByteArrayToHexStr(U8 const* bytes, size_t arrayLen, bool reverse /* = false */)
{
    S32 init = 0;
    S32 end = arrayLen;
    S8 op = 1;
...
void Avalon::Impl::HexStrToByteArray(std::string_view str, U8* out, size_t outlen, bool reverse /*= false*/)
{
    ASSERT(str.size() == (2 * outlen));

    S32 init = 0;
    S32 end = S32(str.length());
    S8 op = 1;

    if (reverse)
    {
        init = S32(str.length() - 2);
        end = -2;
        op = -1;
    }

    U32 j = 0;
    for (S32 i = init; i != end; i += 2 * op)
    {
        char buffer[3] = { str[i], str[i + 1], '\0' };
        out[j++] = U8(strtoul(buffer, nullptr, 16));
    }
}
