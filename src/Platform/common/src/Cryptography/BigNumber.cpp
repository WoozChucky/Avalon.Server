#include <Common/Cryptography/BigNumber.h>
...
void BigNumber::SetDword(S32 val)
{
    SetDword(U32(std::abs(val)));
    if (val < 0)
        BN_set_negative(_bn, 1);
}

void BigNumber::SetDword(U32 val)
{
    BN_set_word(_bn, val);
}

void BigNumber::SetQword(U64 val)
{
    BN_set_word(_bn, (U32)(val >> 32));
    BN_lshift(_bn, _bn, 32);
    BN_add_word(_bn, (U32)(val & 0xFFFFFFFF));
}

void BigNumber::SetBinary(U8 const* bytes, S32 len, bool littleEndian)
{
    if (littleEndian)
    {
...
void BigNumber::SetRand(S32 numbits)
{
...
S32 BigNumber::GetNumBytes() const
{
    return BN_num_bytes(_bn);
}

U32 BigNumber::AsDword() const
{
    return (U32)BN_get_word(_bn);
}
...
void BigNumber::GetBytes(U8* buf, size_t bufsize, bool littleEndian) const
{
#if defined(OPENSSL_VERSION_NUMBER) && OPENSSL_VERSION_NUMBER < 0x10100000L
...
std::vector<U8> BigNumber::ToByteVector(S32 minSize, bool littleEndian) const
{
