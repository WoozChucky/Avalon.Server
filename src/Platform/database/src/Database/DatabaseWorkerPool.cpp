...
#include <Database/Implementation/LoginDatabase.h>
...
template <class T>
void DatabaseWorkerPool<T>::SetConnectionInfo(std::string_view infoString, U8 const asyncThreads, U8 const synchThreads)
{
...
U32 DatabaseWorkerPool<T>::Open()
{
    ...
    U32 error = OpenConnections(IDX_ASYNC, _async_threads);
    ...
    for (auto const& connections : _connections)
    {
        for (auto const& connection : connections)
        {
            ...
            for (size_t i = 0; i < preparedSize; ++i)
            {
                ...
                {
                    U32 const paramCount = stmt->GetParameterCount();

                    // WH only supports uint8 indices.
                    ASSERT(paramCount < std::numeric_limits<U8>::max());

                    _preparedStatementSize[i] = static_cast<U8>(paramCount);
                }
            }
        }
    }
...
void DatabaseWorkerPool<T>::EscapeString(std::string& str)
{
    ...
    EscapeString(buf, str.c_str(), U32(str.size()));
    str = buf;
    delete[] buf;
}
...
template <class T>
U32 DatabaseWorkerPool<T>::OpenConnections(InternalIndex type, U8 numConnections)
{
    for (U8 i = 0; i < numConnections; ++i)
    {
