...
namespace
{
    ...
    constexpr T GetDefaultValue()
    {
        if constexpr (std::is_same_v<T, bool>)
            ...
            return 1.0f;
        else if constexpr (std::is_same_v<T, std::vector<U8>> || std::is_same_v<std::string_view, T>)
            return {};
        else
            return "";
    }
    ...
    {
        // Int8
        if constexpr (std::is_same_v<T, bool> || std::is_same_v<T, S8> || std::is_same_v<T, U8>)
        {
        ...
        // In16
        if constexpr (std::is_same_v<T, U16> || std::is_same_v<T, S16>)
        {
        ...
        // Int64
        if constexpr (std::is_same_v<T, U64> || std::is_same_v<T, S64>)
        {
...
void Field::GetBinarySizeChecked(U8* buf, size_t length) const
{
...
void Field::SetByteValue(char const* newValue, U32 length)
{
...
void Field::SetStructuredValue(char const* newValue, U32 length)
{
...
T Field::GetData() const
{
    ...
    if (data.raw)
        ...
        result = Avalon::StringTo<T>(data.value);
    ...
    if constexpr (std::is_same_v<T, double>)
    {
        if (data.raw && !IsType(DatabaseFieldTypes::Decimal))
            ...
            result = Avalon::StringTo<float>(data.value);
    }
    ...
    if constexpr (std::is_same_v<T, U32>)
    {
        ...
        {
            auto signedResult = Avalon::StringTo<S32>(data.value);
...
template U8 Field::GetData() const;
template U16 Field::GetData() const;
template U32 Field::GetData() const;
template U64 Field::GetData() const;
template S8 Field::GetData() const;
template S16 Field::GetData() const;
template S32 Field::GetData() const;
template S64 Field::GetData() const;
