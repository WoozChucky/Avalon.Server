...
namespace
{
    static U32 SizeForType(MYSQL_FIELD* field)
    {
        switch (field->type)
        {
            ...
            case MYSQL_TYPE_GEOMETRY:
                ...
                */
            default:
                LOG_WARN("sql.sql", "SQL::SizeForType(): invalid field type {}", U32(field->type));
                return 0;
        }
    }
    ...
    DatabaseFieldTypes MysqlTypeToFieldType(enum_field_types type)
    {
        switch (type)
        {
            ...
            default:
                LOG_WARN("sql.sql", "MysqlTypeToFieldType(): invalid field type {}", U32(type));
                break;
        }
    ...
    void InitializeDatabaseFieldMetadata(QueryResultFieldMetadata* meta, MySQLField const* field, U32 fieldIndex)
    {
...
ResultSet::ResultSet(MySQLResult* result, MySQLField* fields, U64 rowCount, U32 fieldCount) :
    _rowCount(rowCount),
...
std::string ResultSet::GetFieldName(U32 index) const
{
...
PreparedResultSet::PreparedResultSet(MySQLStmt* stmt, MySQLResult* result, U64 rowCount, U32 fieldCount) :
    m_rowCount(rowCount),
    m_rowPosition(0),
    ...
    m_metadataResult(result)
{
    ...
    for (U32 i = 0, offset = 0; i < m_fieldCount; ++i)
    {
    ...
    m_rows.resize(U32(m_rowCount) * m_fieldCount);

    while (_NextRow())
    {
        for (U32 fIndex = 0; fIndex < m_fieldCount; ++fIndex)
        {
            m_rows[U32(m_rowPosition) * m_fieldCount + fIndex].SetMetadata(&m_fieldMetadata[fIndex]);
            ...
            if (!*m_rBind[fIndex].is_null)
            {
                ...
                m_rows[U32(m_rowPosition) * m_fieldCount + fIndex].SetByteValue((char const*)buffer, fetched_length);
            ...
            {
                m_rows[U32(m_rowPosition) * m_fieldCount + fIndex].SetByteValue(nullptr, *m_rBind[fIndex].length);
            }
        }
...
Field* PreparedResultSet::Fetch() const
{
    ...
    return const_cast<Field*>(&m_rows[U32(m_rowPosition) * m_fieldCount]);
}
...
Field const& PreparedResultSet::operator[](std::size_t index) const
{
    ...
    return m_rows[U32(m_rowPosition) * m_fieldCount + index];
}
