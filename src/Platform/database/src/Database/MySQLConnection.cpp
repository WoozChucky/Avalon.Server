...
{
    std::vector<std::string_view> tokens = Avalon::Tokenize(infoString, ';', true);
...
U32 MySQLConnection::Open()
{
    ...
    else
    {
        ...
        U32 errorCode = mysql_errno(mysqlInit);
...
bool MySQLConnection::Execute(std::string_view sql)
{
...

    {
        ...
        {
            U32 lErrno = mysql_errno(m_Mysql);
...
bool MySQLConnection::_Query(PreparedStatementBase* stmt, MySQLPreparedStatement** mysqlStmt, MySQLResult** pResult, U64* pRowCount, U32* pFieldCount)
{
    if (!m_Mysql)
        return false;
...
bool MySQLConnection::_Query(std::string_view sql, MySQLResult** pResult, MySQLField** pFields, U64* pRowCount, U32* pFieldCount)
{
    if (!m_Mysql)
        return false;
...
U32 MySQLConnection::GetLastError()
{
...
U32 MySQLConnection::GetServerVersion() const
{
    return mysql_get_server_version(m_Mysql);
}

MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(U32 index)
{
...
void MySQLConnection::PrepareStatement(U32 index, std::string_view sql, ConnectionFlags flags)
{
...
bool MySQLConnection::_HandleMySQLErrno(U32 errNo, U8 attempts /*= 5*/)
{
    switch (errNo)
    {
