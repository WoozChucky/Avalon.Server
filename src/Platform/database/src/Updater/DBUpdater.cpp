...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    {
        exe = Avalon::SearchExecutableInPath("mysql");
...
template<>
bool DBUpdater<LoginDatabaseConnection>::IsEnabled(U32 const updateMask)
{
...
template<>
bool DBUpdater<WorldDatabaseConnection>::IsEnabled(U32 const updateMask)
{
    // This way silences warnings under msvc
...
template<>
bool DBUpdater<CharacterDatabaseConnection>::IsEnabled(U32 const updateMask)
{
    // This way silences warnings under msvc
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool, std::string_view modulesList /*= {}*/)
{
    ...
    {
        auto checkTable = DBUpdater<T>::Retrieve(pool, Avalon::StringFormatFmt("SHOW TABLES LIKE '{}'", tableName));
        if (!checkTable)
        {
    ...
    try
    {
        result = updateFetcher.Update(
                     ...
                     sConfigMgr->GetOption<S32>("Updates.CleanDeadRefMaxCount", 3));
    }
    catch (UpdateException&)
    {
        return false;
    ...
    std::string const info = Avalon::StringFormatFmt("Containing {} new and {} archived updates.", result.recent, result.archived);
                             ...
                             std::string const& password, std::string const& port_or_socket, std::string const& database, std::string const& ssl, Path const& path)
{
    ...
    tempDir = Avalon::AddSuffixIfNotExists(tempDir, std::filesystem::path::preferred_separator);
    ...
    // Execute sql file
    args.emplace_back("-e");
    args.emplace_back(Avalon::StringFormat("BEGIN; SOURCE %s; COMMIT;", path.generic_string().c_str()));
    ...
    int const ret = Avalon::StartProcess(DBUpdaterUtil::GetCorrectedMySQLExecutable(), args,
        "sql.updates", "", true);
