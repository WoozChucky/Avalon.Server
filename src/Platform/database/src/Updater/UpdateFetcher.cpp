...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, U32 const depth) const
{
    static U32 const MAX_DEPTH = 10;
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    else
    {
        ...
        for (auto const& itr : Avalon::Tokenize(_modulesList, ',', true))
        {
...
UpdateFetcher::AppliedFileStorage UpdateFetcher::ReceiveAppliedFiles() const
{
    ...
    do
    {
        ...
        {
            fields[0].Get<std::string>(), fields[1].Get<std::string>(), AppliedFileEntry::StateConvert(fields[2].Get<std::string>()), fields[3].Get<U64>()
        };
...
UpdateResult UpdateFetcher::Update(bool const redundancyChecks,
                                   ...
                                   S32 const cleanDeadReferencesMaxCount) const
{
    ...
    auto ApplyUpdateFile = [&](LocaleFileEntry const& sqlFile)
    {
        ...
        std::string const hash = ByteArrayToHexStr(Avalon::Crypto::SHA1::GetDigestOf(ReadSQLUpdate(filePath)));
...
U32 UpdateFetcher::Apply(Path const& path) const
{
    ...
    return U32(std::chrono::duration_cast<std::chrono::milliseconds>(Time::now() - begin).count());
}

void UpdateFetcher::UpdateEntry(AppliedFileEntry const& entry, U32 const speed) const
{
