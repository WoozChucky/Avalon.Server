...
class ObjectGuid
{
    public:
        ...
        template<HighGuid type>
        static typename std::enable_if<ObjectGuidTraits<type>::MapSpecific, ObjectGuid>::type Create(U32 entry, LowType counter) { return MapSpecific(type, entry, counter); }

        ObjectGuid()  = default;
        explicit ObjectGuid(U64 guid) : _guid(guid) { }
        ObjectGuid(HighGuid hi, U32 entry, LowType counter) : _guid(counter ? U64(counter) | (U64(entry) << 24) | (U64(hi) << 48) : 0) { }
        ObjectGuid(HighGuid hi, LowType counter) : _guid(counter ? U64(counter) | (U64(hi) << 48) : 0) { }

        PackedGuidReader ReadAsPacked() { return PackedGuidReader(*this); }

        void Set(U64 guid) { _guid = guid; }
        void Clear() { _guid = 0; }

        [[nodiscard]] PackedGuid WriteAsPacked() const;

        [[nodiscard]] U64   GetRawValue() const { return _guid; }
        [[nodiscard]] HighGuid GetHigh() const { return HighGuid((_guid >> 48) & 0x0000FFFF); }
        [[nodiscard]] U32   GetEntry() const { return HasEntry() ? U32((_guid >> 24) & UI64LIT(0x0000000000FFFFFF)) : 0; }
    ...
    private:
        ...
        static ObjectGuid MapSpecific(HighGuid type, U32 entry, LowType counter);

        explicit ObjectGuid(U32 const&) = delete;                // no implementation, used to catch wrong type assignment
        ObjectGuid(HighGuid, U32, U64 counter) = delete;      // no implementation, used to catch wrong type assignment
        ObjectGuid(HighGuid, U64 counter) = delete;              // no implementation, used to catch wrong type assignment
...
class PackedGuid
{
    ...
    public:
        ...
        explicit PackedGuid(U64 guid) : _packedGuid(PACKED_GUID_MIN_BUFFER_SIZE) { _packedGuid.appendPackGUID(guid); }
        explicit PackedGuid(ObjectGuid guid) : _packedGuid(PACKED_GUID_MIN_BUFFER_SIZE) { _packedGuid.appendPackGUID(guid.GetRawValue()); }

        void Set(U64 guid) { _packedGuid.wpos(0); _packedGuid.appendPackGUID(guid); }
...
namespace std
{
    ...
    struct hash<ObjectGuid>
    {
        public:
            ...
            {
                return std::hash<U64>()(key.GetRawValue());
            }
    };
}
