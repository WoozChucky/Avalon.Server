...
ObjectGuid ObjectGuid::MapSpecific(HighGuid type, U32 entry, LowType counter)
{
...
{
    buf << U64(guid.GetRawValue());
    return buf;
}

ByteBuffer& operator>>(ByteBuffer& buf, ObjectGuid& guid)
{
    guid.Set(buf.read<U64>());
    return buf;
}
...
{
    buf.readPackGUID(reinterpret_cast<U64&>(guid.Guid));
    return buf;
}
